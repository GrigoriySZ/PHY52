...
def get_logger(file: str=LOG_FILE, encoding: str='utf-8'):
    """Задает начальные настройки логирования

    Returns:
        logger : логгер для записи логов в файл
    """
    ...
    # Создаем обработчика для записи в файл
    file_handler = logging.FileHandler(file, encoding=encoding)
    ...
    # Добавляем обработчика к логгеру
...
def log_action(func):
    ...
    def wrapper(self,*args, **kwargs):
        ...
        log.info(f'Вызов {func.__name__} в {self.__class__.__name__} '
                 f'c аргументами args={args}, kwargs={kwargs}')
        try:

            result = func(self,*args, **kwargs)
            log.info(f'Вызов {func.__name__} в {self.__class__.__name__} '
                     f'вернул результат: {result}')
            return result
        except Exception as e:
            log.error(f'Ошибка функции {func.__name__} в {self.__class__.__name__}: {e}')
    return wrapper
